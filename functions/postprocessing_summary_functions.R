...
find_crude_force_infection_age_recipient <- function(eligible_count, incidence_cases){
  ...
  tmp <- eligible_count_wide[, list(TOTAL_INFECTED = sum(INFECTED)), by = c('SEX', 'COMM', 'PERIOD')]
  tmp[, SEX1 := 'M']
  tmp[SEX == 'M', SEX1 := 'F']
  ...
  tmp <- merge(tmp, incidence_cases, by = c('SEX', 'COMM', 'PERIOD', 'BEFORE_CUTOFF', 'AGEYRS', 'INDEX_TIME'))
  
  # find crude foi
  tmp[, CRUDE_FOI := INCIDENT_CASES / (PROP_SUSCEPTIBLE * TOTAL_INFECTED * PERIOD_SPAN)]
...
find_crude_force_infection_age_recipient_round <- function(incidence_cases_round){
  ...
  eligible_count_wide[, PROP_SUSCEPTIBLE := SUSCEPTIBLE / ELIGIBLE]
  tmp <- eligible_count_wide[, list(TOTAL_INFECTED = sum(INFECTED)), by = c('SEX', 'COMM', 'ROUND')]
  tmp[, SEX1 := 'M']
  tmp[SEX == 'M', SEX1 := 'F']
  set(tmp, NULL, 'SEX', NULL)
  setnames(tmp, 'SEX1', 'SEX')
