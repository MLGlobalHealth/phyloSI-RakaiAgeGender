...
rin <- quest[, .(ageyrs, round, study_id, sex, comm_num, intdate, cuarvmed )]
...
# remove R015-R016 for fishing and R015-R015S for inland no data
rinc <- rinc[!(comm == 'fishing' & round %in% c('R015', 'R015S', 'R016')) & !(comm == 'inland' & round %in% c('R015', 'R015S'))]
...
# save
write.csv(rartfit, file = file.path(indir.deepsequencedata, 'RCCS_R15_R18', "RCCS_nonsuppressed_proportion_220719.csv"), row.names = F)
