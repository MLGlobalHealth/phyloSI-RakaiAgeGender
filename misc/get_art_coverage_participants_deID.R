...
rart <- as.data.table( read.csv(file.path(indir.repository, 'data', 'art_coverage_participants.csv'))  )
...
if(1){
  ...
  ggsave(p, file=file.path(outdir, paste0('count_selfreportedart_by_gender_loc_age_221101.png')), w=8, h=12)
...
ggsave(file=file.path(outdir, paste0('smooth_artcoverage_participants_221101.png')), w=8, h=12)
...
file.name <- file.path(indir.deepsequencedata, 'RCCS_data_estimate_incidence_inland_R6_R18/220903/', paste0('RCCS_art_estimates_221101.csv'))
write.csv(nsinf, file = file.name, row.names = F)

file.name <- file.path(indir.deepsequencedata, 'RCCS_data_estimate_incidence_inland_R6_R18/220903/', paste0('RCCS_art_posterior_samples_221101.csv'))
