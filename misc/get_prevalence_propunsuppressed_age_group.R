...
# AB: This only saves things on the HPC, but I am not sure the outputs are used in any downstream code
...
if(! file.exists(file.name) | config$overwrite.existing.files )
{
    ...
    fwrite(sing.age, file = file.name, row.names = F)
...
{
    cat("Saving file:", file.name, '\n')
    fwrite(sinf.age, file = file.name, row.names = F)
