...
# posterior samples
file.unsuppressedviralload <- file.path(indir.deepsequencedata, 'RCCS_R15_R20', paste0('RCCS_nonsuppressed_proportion_posterior_samples_vl_1000_newlyregistered_221101.csv'))
file.selfreportedart <- file.path(indir.deepsequencedata, 'RCCS_data_estimate_incidence_inland_R6_R18/220903/', paste0('RCCS_art_posterior_samples_newlyregistered_221101.csv'))
...
ggplot(tmp, aes(x = AGEYRS)) + 
  ...
  labs(x = 'Age', y = 'Suppression rate among HIV-positive non-participants', fill = '') +
  theme_bw() +
  theme(legend.position = 'bottom', 
        strip.background = element_rect(colour="white", fill="white"),
        strip.text = element_text(size = rel(1))) +
  scale_y_continuous(limits= c(0,1))
ggsave(file=file.path(outdir, paste0('smooth_suppression_rate_nonparticipants_221101.png')), w=9, h=8)
...
ggplot(tmp, aes(x = AGEYRS)) + 
  ...
  facet_grid(ROUND_LABEL~COMM_LABEL + SEX_LABEL) +
  labs(x = 'Age', y = 'Proportion of HIV-positive non-participants with ART use', fill = '') +
  theme_bw() +
  theme(legend.position = 'bottom', 
        strip.background = element_rect(colour="white", fill="white"),
        strip.text = element_text(size = rel(1))) + 
  scale_y_continuous(labels = scales::percent, limits= c(0,1))
ggsave(file=file.path(outdir, paste0('smooth_art_coverage_nonparticipants_221101.png')), w=9, h=8)
...
ggplot(tmp, aes(x = AGEYRS)) + 
  ...
  facet_grid(ROUND_LABEL~COMM_LABEL + SEX_LABEL) +
  labs(x = 'Age', y = 'Proportion of HIV-positive non-participants with unsuppressed viral load', fill = '') +
  theme_bw() +
  theme(legend.position = 'bottom', 
        strip.background = element_rect(colour="white", fill="white"),
        strip.text = element_text(size = rel(1))) + 
  scale_y_continuous(labels = scales::percent, limits= c(0,1))
ggsave(file=file.path(outdir, paste0('smooth_unsuppressed_proportion_nonparticipants_221101.png')), w=9, h=8)
...
ggplot(tmp, aes(x = AGEYRS)) + 
  ...
  facet_grid(ROUND_LABEL~COMM_LABEL + SEX_LABEL) +
  labs(x = 'Age', y = 'Proportion of HIV-positive non-participants with suppressed viral load', fill = '') +
  theme_bw() +
  theme(legend.position = 'bottom', 
        strip.background = element_rect(colour="white", fill="white"),
        strip.text = element_text(size = rel(1))) + 
  scale_y_continuous(labels = scales::percent, limits= c(0,1))
ggsave(file=file.path(outdir, paste0('smooth_suppressed_proportion_nonparticipants_221101.png')), w=9, h=8)


# save
file.name <- file.path(indir.deepsequencedata, 'RCCS_data_estimate_incidence_inland_R6_R18/220903/', paste0('RCCS_treatment_cascade_nonparticipants_posterior_samples_221101.csv'))
write.csv(df, file = file.name, row.names = F)

file.name <- file.path(indir.deepsequencedata, 'RCCS_data_estimate_incidence_inland_R6_R18/220903/', paste0('RCCS_treatment_cascade_nonparticipants_estimates_221101.csv'))
