...
if(1){
  ...
  ggsave(p, file=file.path(outdir, paste0('count_selfreportedart_by_gender_loc_age_newlyregistered_vl200_221208.pdf')), w=7, h=9)
...
for(i in seq_along(rounds)){
  ...
  # load samples
  if(as.numeric(round) == 15){ # change at round 15
    filename <- paste0('art_gp_stanfit_round',round,'_newlyregistered_vl200_221121.rds')
  }else if(as.numeric(round) >= 16){ # change after round 16 (joseph update art variable)
    filename <- paste0('art_gp_stanfit_round',round,'_newlyregistered_vl200_221208.rds')
  }else{ # use same as 1000ml for rounds 10-14
...
file.name <- file.path(indir.repository, 'fit', paste0('RCCS_art_posterior_samples_newlyregistered_vl200_221208.rds'))
saveRDS(nsinf.samples, file = file.name)

file.name <- file.path(outdir, paste0('RCCS_art_model_fit_newlyregistered_vl200_221208.RDS'))
