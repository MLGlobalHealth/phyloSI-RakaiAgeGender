...
model {
  ...
  for (i in 1:N_DIRECTION){
    ...
    for(k in 2:(N_ROUND_INLAND - 1)){
      log_beta_baseline_contrast_round_inland[k,i] ~ normal(log_beta_baseline_contrast_round_inland[k - 1,i], sigma_beta_baseline_contrast_round[2]);
    }
    for(k in 2:(N_ROUND_FISHING - 1)){
      log_beta_baseline_contrast_round_fishing[k,i] ~ normal(log_beta_baseline_contrast_round_fishing[k - 1,i], sigma_beta_baseline_contrast_round[1]);
    }
