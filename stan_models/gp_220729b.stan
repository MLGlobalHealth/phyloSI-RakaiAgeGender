functions {
  real poisson_times_constant_array_lpmf(int[] y, int n, vector alpha, vector l_lambda) {

   vector[n] z = to_vector(y) ./ alpha;
   // vector[n] l_gamma = lgamma(z + 1);
   vector[n] lambda = exp(l_lambda);

   // return sum((- lambda + z .* l_lambda - l_gamma)[sampled]);
    return sum((- lambda + z .* l_lambda));
  }
  
  real poisson_times_constant_lpmf(int y, real alpha, real l_lambda) {

    real z = y / alpha;
    real l_gamma = lgamma(z + 1);
    real lambda = exp(l_lambda);

   return - lambda + z .* l_lambda - l_gamma;
  }
...
{   
  real delta0 = 1e-9;  
  vector[N_PER_GROUP] prop_sampling[N_DIRECTION, N_COMMUNITY, N_PERIOD] = exp(log_prop_sampling); 
...
transformed parameters {
  ...
  vector[number_rows] log_beta_period[N_DIRECTION];
  ...
  for(i in 1:N_DIRECTION){
    ...
    log_beta_period[i] = BASIS_ROWS' * gp_1D(num_basis_rows, IDX_BASIS_ROWS, delta0, alpha_gp_period[i], rho_gp_period[i], z_period[i]);
    log_beta_period_contrast[i] = rep_vector(log_beta_baseline_period, N_PER_GROUP) + log_beta_period[i][map_age_source];
    ...
    for(j in 1:N_COMMUNITY){
      for(k in 1:N_PERIOD){
        ...
        // add period contrast
        if(k == 2){
          log_beta[i,j,k] += log_beta_period_contrast[i]; 
        }
...
model {
  ...
  for (i in 1:N_DIRECTION){
    ...
    for (j in 1:N_COMMUNITY){
      for (k in 1:N_PERIOD){
         target += poisson_times_constant_array_lpmf(y[sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k],i,j,k] | n_sampling_index_y[i,j,k], prop_sampling[i,j,k][sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k]], log_lambda_latent[i,j,k][sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k]]);
        z[sampling_index_z[1:n_sampling_index_z[i,j,k],i,j,k],i,j,k] ~ poisson_log(log_lambda_latent_recipient[i,j,k][sampling_index_z[1:n_sampling_index_z[i,j,k],i,j,k]]);
      }
    }
  }
}
...
generated quantities{
  real y_predict[N_PER_GROUP, N_DIRECTION, N_COMMUNITY, N_PERIOD];
     ...
     for(j in 1:N_COMMUNITY){
       for(k in 1:N_PERIOD){
         for(n in 1:N_PER_GROUP){
           ...
           y_predict[n,i,j,k] = z_predict[n,i,j,k] * prop_sampling[i,j,k][n];
           if(sampling_index_y[n,i,j,k] != -1){
             log_lik[n,i,j,k] = poisson_times_constant_lpmf(y[n,i,j,k] | prop_sampling[i,j,k][n], log_lambda_latent[i,j,k][n]);
           }else{
