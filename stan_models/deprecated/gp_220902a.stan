...
parameters {
  real log_beta_baseline[N_COMMUNITY];
...
transformed parameters {
  ...
  // start with baseline
  log_beta[:,j,:] = rep_array(rep_vector(log_beta_baseline[j], N_PER_GROUP), N_DIRECTION, N_ROUND);
  ...
  for(i in 1:N_DIRECTION){
    ...
    if(i == 2){
      log_beta_direction_contrast[i,j] +=  rep_vector(log_beta_baseline_contrast_direction[j], N_PER_GROUP);
    }
