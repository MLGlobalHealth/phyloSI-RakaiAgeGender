functions {
  real poisson_times_constant_array_lpmf(int[] y, int n, vector alpha, vector l_lambda) {
   ...
   vector[n] l_gamma = lgamma(z + 1.0);
   vector[n] lambda = exp(l_lambda);

   return sum((- lambda + z .* l_lambda - l_gamma));
...
model {
  ...
  for (i in 1:N_DIRECTION){
    ...
    for (j in 1:N_COMMUNITY){
      for (k in 1:N_PERIOD){
        y[sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k],i,j,k] ~ poisson_times_constant_array( n_sampling_index_y[i,j,k], prop_sampling[i,j,k][sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k]], log_lambda_latent[i,j,k][sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k]]);
