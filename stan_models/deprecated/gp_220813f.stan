...
parameters {
  ...
  real log_beta_baseline_contrast_round[N_ROUND - 1, N_DIRECTION];
  real<lower=0> sigma_beta_baseline_contrast_round;
  real<lower=0> rho_gp_round[N_DIRECTION,N_ROUND - 1];
  real<lower=0> alpha_gp_round[N_DIRECTION,N_ROUND - 1];
  vector[num_basis_rows] z_round[N_DIRECTION, N_ROUND - 1];
...
transformed parameters {
  ...
  for(i in 1:N_DIRECTION){
    ...
    if(i == 2){
     ...
     // find period and round contrast
    ...
    log_beta_round_contrast[i] = rep_matrix(to_vector(log_beta_baseline_contrast_round[:,i]), N_PER_GROUP); 
    for(k in 1:(N_ROUND-1)){
      log_beta_round_contrast[i][k,] += to_row_vector((BASIS_ROWS' * gp_1D(num_basis_rows, IDX_BASIS_ROWS, delta0, alpha_gp_round[i,k], rho_gp_round[i,k], z_round[i,k]))[map_age_recipient] );
    }
    ...
    for(j in 1:N_COMMUNITY){
      for(k in 1:N_ROUND){
        ...
        // add round contrast
        if(k > 1){
          log_beta[i,j,k] += to_vector(log_beta_round_contrast[i][(k-1),:]); 
        }
...
model {
  ...
  for (i in 1:N_DIRECTION){
    ...
    rho_gp_round[i,] ~ inv_gamma(2, 2);
    alpha_gp_round[i,] ~ cauchy(0,1);
    log_beta_baseline_contrast_round[1,i] ~ normal(log_beta_baseline, sigma_beta_baseline_contrast_round);
      for(j in 2:(N_ROUND - 1)){
        log_beta_baseline_contrast_round[j,i] ~ normal(log_beta_baseline_contrast_round[j - 1,i], sigma_beta_baseline_contrast_round);
      }
