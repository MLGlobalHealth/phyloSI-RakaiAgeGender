...
data {
  ...
  int<lower=1, upper=N_group> N_group_without_community;
	int<lower=1, upper=N_group> index_group_without_community[N_group];
	int<lower=1, upper=N_group> community_to_index_group[N_group_without_community, 2];
	//splines
	int number_rows;
	int number_columns;
  int num_basis_rows;
...
parameters {
  real<lower=0> rho_gp1_without_community[N_group_without_community];
  real<lower=0> rho_gp2_without_community[N_group_without_community];
  real<lower=0> alpha_gp_without_community[N_group_without_community];
  real alpha;
  real nu[N_group];
...
transformed parameters {
  real<lower=0> rho_gp1[N_group] = rho_gp1_without_community[index_group_without_community];
  real<lower=0> rho_gp2[N_group] = rho_gp2_without_community[index_group_without_community];
  real<lower=0> alpha_gp[N_group] = alpha_gp_without_community[index_group_without_community];
  
  vector[N_per_group] log_lambda[N_group];  
  vector[N_per_group] lambda[N_group];  
...
model {
  alpha_gp_without_community ~ cauchy(0,1);
  rho_gp1_without_community ~ inv_gamma(5, 5);
  rho_gp2_without_community ~ inv_gamma(5, 5);
  alpha ~ normal(0, 1);
  nu ~ normal(0, 1);
...
generated quantities{
  ...
  real log_lik[N_per_group,N_group_without_community];
  ...
  for(i in 1:N_group_without_community){
    for(j in 1:N_per_group){
      log_lik[j,i] = poisson_lpmf( y[j,community_to_index_group[i,1]] + y[j,community_to_index_group[i,2]] | lambda[community_to_index_group[i,1]][j] + lambda[community_to_index_group[i,2]][j] );
    }
  }
}
