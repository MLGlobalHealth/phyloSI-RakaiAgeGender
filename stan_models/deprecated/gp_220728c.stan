...
parameters {
  ...
  real<lower=0> rho_gp_period1[N_DIRECTION];
  real<lower=0> rho_gp_period2[N_DIRECTION];
...
transformed parameters {
  ...
  matrix[num_basis_rows,num_basis_columns] low_rank_gp_period[N_DIRECTION]; 
  ...
  for(i in 1:N_DIRECTION){
    ...
    low_rank_gp_period[i] = gp(num_basis_rows, num_basis_columns, IDX_BASIS_ROWS, IDX_BASIS_COLUMNS, delta0, alpha_gp_period[i], rho_gp_period1[i], rho_gp_period2[i], z_period[i]);
    log_beta_period[i] = to_vector(((BASIS_ROWS') * low_rank_gp_period[i] * BASIS_COLUMNS)');
    log_beta_period_contrast[i] = rep_vector(log_beta_baseline_period, N_PER_GROUP) + log_beta_period[i];
...
model {
  ...
  rho_gp_period1 ~ inv_gamma(2, 2);
  rho_gp_period2 ~ inv_gamma(2, 2);
  ...
  for (i in 1:N_DIRECTION){
    ...
    for (j in 1:N_COMMUNITY){
      for (k in 1:N_PERIOD){
        y[sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k],i,j,k] ~ poisson_log(log_lambda[i,j,k][sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k]]);
