...
transformed parameters {
  ...
  for(i in 1:N_DIRECTION){
    ...
    for(j in 1:N_COMMUNITY){
      ...
      log_beta_period_contrast[i,j] = (BASIS_ROWS' * gp_1D(num_basis_rows, IDX_BASIS_ROWS, delta0, alpha_gp_period[i,j], rho_gp_period[i,j], z_period[i,j]))[map_age_recipient];
