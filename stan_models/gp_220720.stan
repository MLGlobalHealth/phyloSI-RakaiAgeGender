...
transformed parameters {
  ...
  vector[N_PER_GROUP] f[N_DIRECTION];
  matrix[num_basis_rows,num_basis_columns] beta_direction[N_DIRECTION]; 
  ...
  for(i in 1:N_DIRECTION){
    beta_direction[i] = gp(num_basis_rows, num_basis_columns, IDX_BASIS_ROWS, IDX_BASIS_COLUMNS, delta0,
              alpha_gp[i], rho_gp1[i], rho_gp2[i], z1[i]);
    f[i] = to_vector(((BASIS_ROWS') * beta_direction[i] * BASIS_COLUMNS)');
