functions {
  real poisson_times_constant_array_lpmf(int[] y, int n, int[] sampled, vector alpha, vector l_lambda) {
  ...
  for (i in 1:N_DIRECTION){
    ...
    for (j in 1:N_COMMUNITY){
      for (k in 1:N_PERIOD){
        ...
        target += poisson_times_constant_array_lpmf(y[:,i,j,k] | N_PER_GROUP, sampling_index_y[1:n_sampling_index_y[i,j,k],i,j,k], prop_sampling[i,j,k], log_lambda_latent[i,j,k]);
