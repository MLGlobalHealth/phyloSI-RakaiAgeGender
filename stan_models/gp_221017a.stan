  ...
  for (i in 1:N_DIRECTION){
    ...
    for(j in 1:N_COMMUNITY){
      ...
      for (k in 1:N_ROUND[j]){
        ...
        log_lambda_latent_peryear_recipient[i,j,k] ~ normal(ir_lognorm_mean[:,i,j,k], ir_lognorm_sd[:,i,j,k]);
      }
        ...
        for(n in 1:N_PER_GROUP){
          for(k in 1:max(N_ROUND)){
            if(k > N_ROUND[j]){
              ...
              log_lik[index] = normal_lpdf(log_lambda_latent_peryear_recipient[i,j,k]|ir_lognorm_mean[:,i,j,k], ir_lognorm_sd[:,i,j,k]) / N_AGE;
