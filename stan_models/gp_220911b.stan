...
data {
  ...
  int z[N_AGE, N_DIRECTION, N_COMMUNITY, max(N_ROUND)]; // incidence cases 
  vector[N_PER_GROUP] log_offset[N_DIRECTION, N_COMMUNITY, max(N_ROUND)]; 
...
parameters {
  ...
  real log_beta_baseline_contrast_round_inland[N_ROUND_INLAND - 1, N_DIRECTION];
  real log_beta_baseline_contrast_round_fishing[N_ROUND_FISHING - 1, N_DIRECTION];
  ...
  real<lower=0> rho_gp_period1[N_DIRECTION,N_COMMUNITY];
  real<lower=0> rho_gp_period2[N_DIRECTION,N_COMMUNITY];
  real<lower=0> alpha_gp_period[N_DIRECTION,N_COMMUNITY];
  matrix[num_basis_rows,num_basis_columns] z_period[N_DIRECTION,N_COMMUNITY];
...
transformed parameters {
  ...
  matrix[num_basis_rows,num_basis_columns] low_rank_gp_period[N_DIRECTION, N_COMMUNITY]; 
  ...
  for(i in 1:N_DIRECTION){
    ...
    for(j in 1:N_COMMUNITY){
      ...
      low_rank_gp_period[i,j] = gp(num_basis_rows, num_basis_columns, IDX_BASIS_ROWS, IDX_BASIS_COLUMNS, delta0,
              alpha_gp_period[i,j], rho_gp_period1[i,j], rho_gp_period2[i,j], z_period[i,j]);
      log_beta_period_contrast[i,j] = to_vector(((BASIS_ROWS') * low_rank_gp_period[i,j] * BASIS_COLUMNS)');
      ...
      if(j == 1){
        log_beta_round_contrast[i,j] += append_row(rep_matrix(to_vector(log_beta_baseline_contrast_round_fishing[:,i]), N_PER_GROUP), 
                                          rep_matrix(rep_row_vector(0.0, N_PER_GROUP),  max(N_ROUND) - N_ROUND_FISHING ));  
      }else{
        log_beta_round_contrast[i,j] += rep_matrix(to_vector(log_beta_baseline_contrast_round_inland[:,i]), N_PER_GROUP) ;
      }
...
model {
  ...
  for(r in 1:num_basis_rows){
    for(c in 1:num_basis_columns){
      z1[:,r,c] ~ normal(0,1);
      for (i in 1:N_DIRECTION){
         for(j in 1:N_COMMUNITY){
          z_period[i,j][r,c] ~ normal(0,1);
        }
      }
    }
  }
  ...
  for (i in 1:N_DIRECTION){
    ...
    rho_gp_period1[i] ~ inv_gamma(2, 2);
    rho_gp_period2[i] ~ inv_gamma(2, 2);
    
    alpha_gp_community[i] ~ cauchy(0,1);
    rho_gp_community[i] ~ inv_gamma(2, 2);
    z_community[i] ~ normal(0,1);
  
    log_beta_baseline_contrast_round_inland[1,i] ~ normal(0, 10);
    log_beta_baseline_contrast_round_fishing[1,i] ~ normal(0, 10);
    for(k in 2:(N_ROUND_INLAND - 1)){
      log_beta_baseline_contrast_round_inland[k,i] ~ normal(log_beta_baseline_contrast_round_inland[k - 1,i], sigma_beta_baseline_contrast_round);
    }
    for(k in 2:(N_ROUND_FISHING - 1)){
      log_beta_baseline_contrast_round_fishing[k,i] ~ normal(log_beta_baseline_contrast_round_fishing[k - 1,i], sigma_beta_baseline_contrast_round);
    }
        ...
        for(n in 1:N_PER_GROUP){
          for(k in 1:max(N_ROUND)){
            if(k > N_ROUND[j]){
              ...
              log_lik[index] = poisson_log_lpmf( z[map_age_source[n],i,j,k] | log_lambda_latent_recipient[i,j,k][map_age_source[n]] ) / N_AGE;
