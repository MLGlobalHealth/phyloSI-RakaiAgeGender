...
transformed parameters {
  ...
  for(i in 1:N_DIRECTION){
    ...
    if(i == 2){
      log_beta_direction_contrast[i,j] += rep_vector(log_beta_baseline_contrast_direction, N_PER_GROUP);
    }
...
model {
  ...
  for (i in 1:N_DIRECTION){
    ...
    for(j in 1:N_COMMUNITY){
       ...
       for (k in 1:N_ROUND){
        z[:,i,j,k] ~ neg_binomial_2_log(log_lambda_latent_recipient[i,j,k], phi);
      }
      
      for (p in 1:N_PERIOD){
        y[sampling_index_y[1:n_sampling_index_y[i,j,p],i,j,p],i,j,p] ~ neg_binomial_2_log(log_lambda[i,j,p][sampling_index_y[1:n_sampling_index_y[i,j,p],i,j,p]], phi);
      }
    }
  }
}
  ...
  for(i in 1:N_DIRECTION){
     for(j in 1:N_COMMUNITY){
       for(n in 1:N_PER_GROUP){
         for(k in 1:N_ROUND){
           z_predict[n,i,j,k] = neg_binomial_2_log_rng(log_lambda_latent[i,j,k][n], phi);
           log_lik[n,i,j,k] = neg_binomial_2_log_lpmf( z[map_age_source[n],i,j,k] | log_lambda_latent_recipient[i,j,k][map_age_source[n]], phi) / N_AGE;
            if(sampling_index_y[n,i,j,map_round_period[k]] != -1){
              log_lik[n,i,j,k] += neg_binomial_2_log_lpmf( y[n,i,j,map_round_period[k]] | log_lambda[i,j,map_round_period[k]][n] , phi) / N_ROUND_PER_PERIOD[map_round_period[k]];
            }
         }
         for(p in 1:N_PERIOD){
           y_predict[n,i,j,p] = neg_binomial_2_log_rng(log_lambda[i,j,p][n], phi);
         }
       }
     }
  }
