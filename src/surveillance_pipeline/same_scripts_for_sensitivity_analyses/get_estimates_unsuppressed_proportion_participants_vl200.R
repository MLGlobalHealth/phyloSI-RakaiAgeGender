...
if (1) {
  ...
  ggsave(p,
         file = file.path(outdir, paste0("count_unsuppressed_by_gender_loc_age_vl200_221121.pdf")),
         w = 7,
         h = 5.2)
}
...
for(i in seq_along(rounds)){
  ...
  nsinf.by.age <- tmp[, list(q = quantile(value, prob = ps, na.rm = TRUE),
                             q_label = qlab),
                       by = c("SEX", "LOC", "AGE_LABEL")]
  nsinf.by.age <- as.data.table(
    reshape2::dcast(nsinf.by.age, ... ~ q_label, value.var = "q")
  )
  ...
  tmp <- merge(tmp,
               DT[, .(SEX, LOC, AGE_LABEL, HIV_N)],
               by = c("SEX", "LOC", "AGE_LABEL"),
               all.x = TRUE)

  # predict count and predict unsuppressed
  tmp[!is.na(HIV_N),
      COUNT_PREDICT := rbinom(1, HIV_N, value),
      by = c("SEX", "LOC", "AGE_LABEL", "iterations")]
  tmp[, UNSUPPRESSED_PREDICT := COUNT_PREDICT / HIV_N]

  # summarise
  nspred.by.age <- tmp[, list(q = quantile(UNSUPPRESSED_PREDICT, prob=ps, na.rm = TRUE),
                             q_label = qlab),
                        by = c("SEX", "LOC", "AGE_LABEL")]
  nspred.by.age <- as.data.table(
    reshape2::dcast(nspred.by.age, ... ~ q_label, value.var = "q")
  )
  ...
  setnames(
    nsinf.by.age, 
    c("LOC_LABEL", "SEX_LABEL", "AGE_LABEL", "EMPIRICAL_VLNS_IN_HIV", "M", "CL", "CU"), 
...
# stats
file.name <- file.path(outdir,"RCCS_nonsuppressed_proportion_model_fit_vl200_230703.RDS")
if (! file.exists(file.name) | config$overwrite.existing.files) {
    cat("Saving file:", file.name, "\n")
