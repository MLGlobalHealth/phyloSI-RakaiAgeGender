...
# for round 10, set art coverage and unsuppressed as round 11
# as questions art related q were introduced from roudn 11
tmp <- df[ROUND == "R011"]
tmp[, ROUND := "R010"]
...
df[, `:=`(SUPPRESSION_RATE_POSTERIOR_SAMPLE = PROP_SUPPRESSED_POSTERIOR_SAMPLE / PROP_ART_COVERAGE_POSTERIOR_SAMPLE, # nolint: line_length_linter.
          SUPPRESSION_RATE_EMPIRICAL = (1 - PROP_UNSUPPRESSED_EMPIRICAL) / PROP_ART_COVERAGE_EMPIRICAL)] # nolint: line_length_linter.

# add constraint that suppression rate must at maximum 1
df[SUPPRESSION_RATE_POSTERIOR_SAMPLE > 1, SUPPRESSION_RATE_POSTERIOR_SAMPLE := 1]  # nolint: line_length_linter.
...
df[ROUND %in% c("R015"), SUPPRESSION_RATE_POSTERIOR_SAMPLE := SUPPRESSION_RATE_POSTERIOR_SAMPLE_R016,
    by = c("AGEYRS", "SEX", "COMM", "iterations")]
df[ROUND %in% c("R015"), SUPPRESSION_RATE_EMPIRICAL := SUPPRESSION_RATE_EMPIRICAL_R016,
    by = c("AGEYRS", "SEX", "COMM", "iterations")]

# find art coverage with the same suppression rate as round 16
df[ROUND %in% c("R015"), PROP_ART_COVERAGE_POSTERIOR_SAMPLE := min(1, PROP_SUPPRESSED_POSTERIOR_SAMPLE / SUPPRESSION_RATE_POSTERIOR_SAMPLE),
    by = c("AGEYRS", "SEX", "COMM", "ROUND", "iterations")]
...
if (1) {
  ...
  df[ROUND %in% c("R010", "R011", "R012", "R013", "R014", "R015S"), 
     PROP_SUPPRESSED_POSTERIOR_SAMPLE := PROP_ART_COVERAGE_POSTERIOR_SAMPLE * SPEC_M + (1 - PROP_ART_COVERAGE_POSTERIOR_SAMPLE) * (1-SENS_M)]
...
if (! file.exists(file.name) || config$overwrite.existing.files) {
  cat("Saving file:", file.name, "\n")
  write.csv(ns, file = file.name, row.names = FALSE)
} else {
  cat("File:", file.name, "already exists...\n")
}
