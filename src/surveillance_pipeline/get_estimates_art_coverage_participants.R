...
outdir <- file.path("../phyloSI-RakaiAgeGender-outputs","get_estimates_art_coverage_participants")
if(usr == 'melodiemonod'){
  outdir <- file.path("/Users/melodiemonod/Box Sync/2023//phyloSI-RakaiAgeGender-outputs","get_estimates_art_coverage_participants")
}
if (!dir.exists(outdir)) dir.create(outdir, recursive = TRUE)

file.exists(c(
  path_stan_binomialgp_binomialgp ,
  path_binomialgp_model_config ,
  path.participant.art ))  |> all() |> stopifnot()
...
# samples
file_name <- file.selfreportedart
