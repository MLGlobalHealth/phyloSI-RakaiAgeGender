...
shrink.intervals <- function(DT)
{
    ...
    while(cond)
    {
        ...
        DT <- out0[['D']]; cond0 <- out0[['C']]
        out1 <- .shrink.based.on.recipient(chain, DT) 
        DT <- out1[['D']]; cond1 <- out1[['C']]
        cat(cond0, cond1, '\n')
        cond <- cond0 | cond1
        iter <- iter + 1
    }
...
{
    ggplot(data=DT) + 
        geom_rect(data=drounds, aes(xmin=min_sample_date, xmax=max_sample_date, fill=round,
                                    ymin=min_sample_date, ymax=max_sample_date), alpha=.5) +
        ...
        labs(title='Comparison of attributed recipient doi',
...
get.transmission.cluster.ids <- function(DT, check=F)
{
        ...
        if(check)
        {
                ...
                .f <- function(x){ tmp1[id==x, uniqueN(GROUP)]}
...
get.polygon.centroid <- function(COHORDS, usenames=FALSE, as.dt=FALSE)
{
        ...
        x_cohords <- COHORDS[,1] |> unlist() |> unname()
        y_cohords <- COHORDS[,2] |> unlist() |> unname()
                                      ...
                                      df_round=data.frame() )
{
    ...
    if( !is.null(importance_weights) & !is.null(range_gi) )
    {
        .get.gi.probs <- function(gi)
        {
            ...
            setnames(out, 'V1', paste0('GI', gi)); out
        }
