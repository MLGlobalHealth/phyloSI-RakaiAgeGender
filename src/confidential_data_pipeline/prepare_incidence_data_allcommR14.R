...
hivstatus_vlcopies_raw <- read_hiv_data_230703(file.path.hiv.614,
                                               file.hiv_R09_R14,
                                               file.path.hiv.1518,
                                               file.path.hiv_19)

# load HIV verification data set
verif_raw <- read_flow_data_230703(file.path.flow.614,
                                   file.path.flow_914,
                                   file.path.flow,
                                   file.path.flow_19)
...
# statistics
file.name <- file.path(outdir,"incidence_inland_statistics_cohort_for_paper_230703.rds")
if(! file.exists(file.name))
{
  cat("\n Saving output file", file.name, "\n")
...
if( !file.exists(file.name))
{
  ...
  #   saveRDS(seroconverter_cohort_list, file = file.name)
}else{
  cat('\n Output file', file.name,'already exists.\n')
}
