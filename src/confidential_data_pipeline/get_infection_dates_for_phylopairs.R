...
if (file.exists(filename_net) & !args$rerun == TRUE) {
    ...
    provide_samples <- lapply(dcohords[, seq(max(N_IDS))], function(x) {
        runif(1e6, 0, 1)
    })
    ...
    Reduce(
        "merge",
        list(
            tmp[NAME == "CENTROID", .(GROUP, CENTROIDS = out)],
...
if (args$confidential) { # compute 95% ranges
...
if (nrow(df_round_gi)) { # compute probability of assigning source to different period
    ...
    dprobs_roundallocation[, (cols) := lapply(.SD, function(x) {
        x[is.na(x)] <- 0
        x
    }), .SDcols = cols]
