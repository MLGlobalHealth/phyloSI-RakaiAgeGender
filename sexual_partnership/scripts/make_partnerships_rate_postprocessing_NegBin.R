...
set(pd, NULL, 'value_m', pd[, exp(log_random_effect_baseline + value + log(T))])
...
pds.r[, cntct_rate_empirical := y/N/T]
...
if (args$if_save_fig)
{
  ...
  pm <- pm + ggtitle(paste0('Age dist with emprical contact rates using ', model_id, " - FM" ))
...
if (args$if_save_fig)
{
  pm <- plot_contact_rate_matrix_emprical(dm)
...
if (args$if_save_fig)
{
  ...
  cat("\nSave posterior predictive mariginal contact to file ", tmp)
  ggsave(file = tmp, pm, width = 6, height = 5)
