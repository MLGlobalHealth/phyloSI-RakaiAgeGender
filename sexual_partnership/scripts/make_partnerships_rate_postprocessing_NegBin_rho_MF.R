...
if ( grepl("BSGP", args$stan_model))
{
  ...
  model_id <- paste0('NegBin_BSGP_knots-', args$n_knots, '-comm-', args$community)
}
if ( grepl("HSGP", args$stan_model))
{
  su.target.vars <- c('log_random_effect_baseline','overdispersion','gp_rho_age1','gp_rho_age2','gp_alpha','z', 'f_mf')
  model_id <- paste0('NegBin_HSGP_c-',args$hsgp_boundary_inflation*100,"_m-",args$hsgp_m, '-comm-', args$community)
}
...
tmp2 <- subset(reported.partnerships, part.sex == 'M' & cont.sex == 'F', select = c(mf_mat_idx, pop))
pd <- merge(pd, tmp2, by = 'mf_mat_idx')
set(pd, NULL, 'value_m', pd[, exp(log_random_effect_baseline + value + log(pop))])

set(pd, NULL, 'value_r', pd[, exp(log_random_effect_baseline + value)])
tmp <- pd[,
          list(sd = sd(value_m),
               value_m = quantile(value_m, p = pds.quantiles),
               stat = pds.quantilelabels),
          by = c('mf_mat_idx')
          ]
tmp <- dcast.data.table(tmp, mf_mat_idx + sd~stat, value.var = 'value_m')
tmp2 <- subset(reported.partnerships, part.sex == 'M' & cont.sex == 'F', select = c(part.sex, cont.sex, part.age, cont.age, mf_mat_idx, capped_cntcts, part, pop, rho))
pds <- merge(tmp, tmp2, by = 'mf_mat_idx')
tmp3 <- pd[,
          list(sd = sd(value_r),
               value_r = quantile(value_r, p = pds.quantiles),
               stat = pds.quantilelabels),
          by = c('mf_mat_idx')
          ]
tmp3 <- dcast.data.table(tmp3, mf_mat_idx + sd~stat, value.var = 'value_r')
pds.r <- merge(tmp3, tmp2, by = 'mf_mat_idx')
...
pd <- merge(pd, tmp2, by = 'iterations')
tmp2 <- subset(reported.partnerships, part.sex == 'F' & cont.sex == 'M', select = c(mf_mat_idx, pop))
pd <- merge(pd, tmp2, by = 'mf_mat_idx')
...
tmp3 <- dcast.data.table(tmp3, mf_mat_idx + sd~stat, value.var = 'value_r')
tmp3 <- merge(tmp3, tmp2, by = 'mf_mat_idx')
...
if (args$if_save_fig)
{
  ...
  pm <- pm + ggtitle(paste0('Age dist with empirical contact intensities using ', model_id, " - MF" ))
...
if (args$if_save_fig)
{
  ...
  pm <- pm + ggtitle(paste0('Age dist with empirical contact intensities using ', model_id, " - FM" ))
...
{
  cat("\nPlot age posterior distribution ...")
  # we just visualize the contact rate for male-female
  pm <- plot_age_contact_rate_MF(pds.r)
  pm <- pm + ggtitle(paste0('Age dist using ', model_id, " - MF" ))

  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-MF.pdf"))
  cat("\nSave age posterior contact rate to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-MF.png"))
  cat("\nSave age posterior contact rate to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
...
if (args$if_save_fig)
{
  ...
  pm <- plot_age_contact_rate_MF_empirical(pds.r)
  pm <- pm + ggtitle(paste0('Age dist with empirical contact intensities using ', model_id, " - MF" ))

  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-MF-with-empirical.pdf"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-MF-with-empirical.png"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
...
if (args$if_save_fig)
{
  cat("\nPlot age posterior distribution ...")
  # we just visualize the contact intensities for female-male
  pm <- plot_age_contact_rate_FM(pds.r)
  pm <- pm + ggtitle(paste0('Age dist using ', model_id, " - FM" ))

  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-FM.pdf"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-FM.png"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
...
if (args$if_save_fig)
{
  ...
  cat("\nPlot age posterior distribution with empirical contact rates ...")
  # we just visualize the contact intensities for female-male
  pm <- plot_age_contact_rate_FM_empirical(pds.r)
  pm <- pm + ggtitle(paste0('Age dist with empirical contact rates using ', model_id, " - FM" ))

  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-FM-with-empirical.pdf"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
  tmp <- file.path(paste0(outfile.base,"_age-posterior-dist-contact-rate-FM-with-empirical.png"))
  cat("\nSave age posterior contact rates to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
...
if (args$if_save_fig)
{
  ...
  pm <- plot_contact_rate_matrix(pds.r) + labs(x = "Age of participant", y = "Age of contact", fill = "contact rate" )
  pm <- pm + theme(legend.text = element_text(angle = 45, vjust = 0.5, hjust = 1))
  pm <- pm + ggtitle(paste0('Posterior mean contacts rate using ', model_id ))
  tmp <- file.path(paste0(outfile.base,"_posterior-contact-rate-patterns.pdf"))
  cat("\nSave posterior contact rate to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 12)
  tmp <- file.path(paste0(outfile.base,"_posterior-contact-rate-patterns.png"))
  cat("\nSave posterior contact rate to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 12)
}
...
tmp.emp <- subset(pds.r, select = c('part.sex', 'cont.sex', 'part.age', 'cont.age', 'cntct_rate_empirical'))
setnames(tmp.emp, 'cntct_rate_empirical', 'M')
set(tmp.emp, NULL, "label", "empirical value")
set(pds.r, NULL, "label", model_id)
dm <- rbind(pds.r, tmp.emp, fill = TRUE)
if (args$if_save_fig)
{
  pm <- plot_contact_rate_matrix_empirical(dm)
  pm <- pm +  ggtitle(paste0('Contact rates patterns comparison using ', model_id  ))
  tmp <- file.path(paste0(outfile.base,"_posterior-contact-rate-patterns-with-truth.pdf"))
  cat("\nSave posterior contact rates with ground truth to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
  tmp <- file.path(paste0(outfile.base,"_posterior-contact-rate-patterns-with-truth.png"))
  cat("\nSave posterior contact rates with ground truth to file ", tmp)
  ggsave(file = tmp, pm, width = 12, height = 8)
}
...
for (i in pds.midx)
{
  ...
  if (gender_i == 'F')
  {
    ...
    tmp <- subset(dcm, m_index == i, select = c(m_index, cont.sex, part.sex, cont.age, part.age, mf_mat_idx, pop))
    pd <- merge(pd, tmp, by = 'mf_mat_idx')
    pd[, log_contact_intensity := log_random_effect_baseline + f_mf + log(pop)]
    set(pd, NULL, c('m_index','col_idx','mf_mat_idx','log_random_effect_baseline','f_mf','log_contact_rate','pop'), NULL)
    pds[[i]] <- copy(pd)
...
#   here we will process the Monte Carlo output in batches to avoid handling tables with >100m rows
cat("\nPosterior predictive check on age and gender specific contact intensities ...\n")
dcm <- subset(reported.partnerships, part > 0 & part.sex != cont.sex)
setkey(dcm, part.sex, part.age)
tmp <- unique(subset(dcm, select = c(part.sex, part.age)))
tmp[, m_index := seq_len(nrow(tmp))]
dcm <- merge(dcm, tmp, by = c('part.sex', 'part.age'))
...
for (i in pds.midx)
{
  ...
  if (gender_i == 'F')
  {
    ...
    pd <- merge(pd, tmp2, by = 'iterations')
    tmp <- subset(dcm, m_index == i, select = c(m_index, part.sex, cont.sex, part.age, cont.age, mf_mat_idx, part, pop, rho))
    pd <- merge(pd, tmp, by = 'mf_mat_idx')
    pd[, log_exp_contacts := log_random_effect_baseline + f_mf + log(part) + log(pop) + log(rho)]
    set(pd, NULL, c('m_index','col_idx','mf_mat_idx','log_random_effect_baseline','f_mf','part', 'pop', 'rho'), NULL)
    pds[[i]] <- copy(pd)
...
tmp <- subset(dcm, select = c('part.sex','cont.sex','part.age','cont.age','capped_cntcts'))
...
pds[, if_out := (capped_cntcts < CL | CU < capped_cntcts)]
...
tmp <- dcm[part > 0,
           list(y_marginal = sum(capped_cntcts),
                m_marginal = sum(capped_cntcts/part),
                part = unique(part)),
           by = c('part.sex','part.age')
]
