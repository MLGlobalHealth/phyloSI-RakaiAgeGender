...
{
  su.target.vars <- c('log_random_effect_baseline', 'log_random_effect_community', 'overdispersion','gp_rho_age1','gp_rho_age2','gp_alpha','z', 'f_mf')
...
p <- model_fit$draws() %>%
  ...
  setNames(c('log_baseline', paste0('log_baseline_comm_', unique(reported.partnerships$part.comm_num)))) %>%
  mcmc_areas()
...
cat("\nSave posterior distribution to file ", tmp)
ggsave(file = tmp, p, width = 6, height = 2 * (1 + length(unique(reported.partnerships$part.comm_num))), limitsize = FALSE)
...
tmp2 <- subset(reported.partnerships, part.sex == 'F', select = c(mf_mat_idx, pop, c))
pd <- merge(pd_save, tmp2, by = c('mf_mat_idx', 'c'))
