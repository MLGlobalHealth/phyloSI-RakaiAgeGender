...
if ( grepl("BSGP", args$stan_model))
{
  ...
  model_id <- paste0('Poisson_BSGP_knots-', args$n_knots, '-comm-', args$community)
}
if ( grepl("HSGP", args$stan_model))
{
  su.target.vars <- c('log_random_effect_baseline','gp_rho_age1','gp_rho_age2','gp_sigma','z', 'f_mf')
  model_id <- paste0('Poisson_HSGP_c-',args$hsgp_boundary_inflation*100,"_m-",args$hsgp_m, '-comm-', args$community)
}
