...
file.meta.R1518 <- file.path(indir.deepsequencedata, 'R019_VoIs', 'quest_R019_VoIs_220607.csv')
file.path.hiv <- file.path(indir.deepsequencedata, 'R019_VoIs', 'HIV_R019_VOIs_220607.csv')

file.community.keys <- file.path(indir.deepsequence_analyses,'community_names.csv')
file.census.eligible.individuals.count <- file.path(indir.deepsequencedata, 'R019_VoIs', 'RCCS_census_eligible_individuals_220621.csv')
...
meta[, birth_date := as.Date(birthdat, '%d%b%Y')]
...
tmp <- as.data.table( reshape2::melt(tmp, id.vars = c('pt_id', 'birth_date', 'visit_date', 'sex', 'comm_num', 'round', 'sexyear', 
                                                      'sexp1yr', 'sexp1out', 'cuarvmed')) )
...
ragem[is.na(part.hiv) & round == 'R019',length(unique(pt_id))] #
...
# save
tmp <- rage[part.round == 'R019']
write.csv(tmp, file = file.path(indir.deepsequencedata, 'R019_VoIs', 'RCCS_reported_partnership_220621.csv'), row.names = F)
