...
data
{
  // int<lower=1> A1; // number of contacting individuals' age inputs + imputing age
  int<lower=1> A2; // number of contacted individuals' age inputs
  int<lower=1> Nc; // number of communities

  // array[A1] real age1; // age of contacting individuals
  // array[A2] real age2; // age of contacted individuals

  int<lower=0> Nmf; // number of observed male-male contacts for all communities
  ...
  vector[Nmf] log_rho_mf;
  // vector[Nmf] log_comm_mf; // within community partners proportion

  array[Nmf] int<lower=1> community_index;
  ...
  matrix[M_age1, A2] basis_age1; // B-splines basis functions on the age of participants (rows)
  matrix[M_age2, A2] basis_age2; // B-splines basis functions on the age of contacted individuals (columns)
...
parameters
{
  real log_random_effect_baseline;
  vector[Nc] log_random_effect_community;
  ...
  real<lower=0> log_random_effect_community_sd;
...
model
{
  ...
  target += normal_lpdf(log_random_effect_community | 0, log_random_effect_community_sd);
  target += exponential_lpdf(log_random_effect_community_sd | 1);

  // model in local scope
  {
    vector[Nmf] log_mu = log_random_effect_baseline + log_random_effect_community[community_index] + f_mf[ymf_rowmajor_matrix_index] + log_pop_mf + log_participants_mf + log_rho_mf;

    // add the effect at community level
