...
parameters
{
 ...
 real<lower=0> gp_rho_age1; // GP hyperparameters
  ...
  matrix[M_age1, M_age2] z; // HSGP basis function coefficients for mf (which is symmetric to fm)
}
...
model
{
  ...
  // model in local scope
  {
    ...
    target += neg_binomial_lpmf( y | exp(log_mu) / overdispersion, inv(overdispersion));
  }
}
...
generated quantities
{
  array[N] real y_pred;
  {
    ...
    vector[N] log_mu =
      ...
      for(i in 1:N)
      {
          y_pred[i] = neg_binomial_lpmf( y[i] | exp(log_mu[i]) / overdispersion, inv(overdispersion));
      }
  }
